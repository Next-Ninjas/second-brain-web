...
export default function Page() {
  ...
  return (
    <div className="h-screen flex flex-col md:grid md:grid-cols-[minmax(0,16rem)_1fr] md:grid-rows-[auto_1fr] bg-background text-foreground">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 bg-green-200 text-black dark:text-white dark:bg-green-950  transition-all duration-300 flex flex-col md:static md:z-auto md:row-span-2 ${
          mobileSidebarOpen
            ? "translate-x-0 w-64"
      ...
      >
        ...
        <nav className="mt-4 space-y-2 px-2">
          ...
          >
            <Plus size={20} />
            {!collapsed && (
              <span className="font-extrabold cursor-pointer">New chat</span>
            )}
            {/* Removed "New Chat" text */}
          </button>
        ...
        <div className="px-2 flex-1 overflow-y-auto">
          {!collapsed && (
            <h3 className="text-sm font-semibold mb-2 sticky top-0  py-2 z-10">
              Recent Sessions
            </h3>
          )}
              ...
              <li key={session.id} className="group relative">
                ...
                >
                  <div className="flex-1 truncate">
                    {!collapsed ? (
                      <>
                        {/* Display message content instead of title */}
      ...
      <main className="w-full flex flex-col min-h-0 md:col-start-2 md:row-start-2">
                  ...
                  {sessionMessages.map((msg) => (
                    <div
                      ...
                      className={`p-4 rounded-lg max-w-3xl mx-auto whitespace-pre-wrap break-words ${
                        msg.role === "user"
                          ? "bg-blue-100 dark:bg-gray-800 ml-auto"
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t  md:border-t-0 px-4 py-3 md:sticky md:bottom-5">
          <div className="max-w mx-auto">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-muted rounded-xl px-4 py-2">
              ...
              >
                <Send className="h-5 w-5 text-green-900 dark:text-green-200" />
              </Button>
            </div>
